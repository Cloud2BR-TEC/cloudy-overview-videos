...
Cloudy Repository Video Studio is a static GitHub Pages application with two formats. Cloudy YouTube Overview Videos creates an editable long-form storyboard from repository documentation, while Cloudy Shorts turns a selected documented topic into a one-minute story with a compact narration sequence and a reusable production-asset library. Paste a public GitHub repository URL to create either format from repository metadata, README content, and images discovered across the repository. Cloudy can preview narration using a locally available female voice when the browser provides one. Editing remains in the current browser session; project setup, SRT captions, and WebM video downloads unlock after all export requirements are complete.
...
Cloudy Shorts selects five related documented scenes from the topic's section to make a one-minute narrative. Its production library combines Cloudy, the selected topic, authored repository visuals, and repository topics, giving each short a grounded set of host, concept, and visual assets. Shorts can preview narration locally and download a Markdown production script with the narration, story beats, and selected assets.
