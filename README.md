...
Cloudy reads public GitHub API data to assemble the storyboard. Slide bullets and narration are audience-facing summaries grounded in repository metadata, the main README, English or default-language Markdown under `docs/`, and eligible repository paths; localized non-English docs are ignored. Internal editorial directions are never shown as presentation content. Each slide reserves different evidence, and substantially similar slide content fails validation instead of being repeated. Repository planning files named `agenda.yml` or `agenda.yaml` are excluded. Cloudy does not write to repositories or upload content. Private repositories cannot be used in this browser-only version.
